...
ツールバーの「レポート」は、字数・指摘件数の集計と各指摘の一覧をMarkdownファイル(`kousei-report.md`)として書き出す。レビューの共有や記録に使える。
...
- `src/lib/rules/variants.ts` 表記ゆれ34グループ。文書全体の集計と熟語境界の除外
...
- `src/lib/report.ts` 指摘と統計をMarkdownレポートへ整形
...
- 表記ゆれの辞書は技術文書・ビジネス文書で頻出の34グループに絞っている。固有の用語集を読み込む機能はない。
